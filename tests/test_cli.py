...
class TestCLIScore:
    def test_json_output(self):
        result = run_cli("score", SAMPLE_RULES)
    ...
    def test_csv_output(self):
        result = run_cli("score", SAMPLE_RULES, "--format", "csv")
        assert result.returncode == 0, result.stderr
    ...
    def test_verbose_json(self):
        result = run_cli("score", SAMPLE_RULES, "--verbose")
        assert result.returncode == 0, result.stderr
        data = json.loads(result.stdout)
        assert "matched_criteria" in data[0]

    def test_stats_output(self):
        result = run_cli("score", SAMPLE_RULES, "--stats")
        assert result.returncode == 0
        assert "Scoring Summary" in result.stderr

    def test_custom_config(self):
        result = run_cli("score", SAMPLE_RULES, "--config", TEST_PROFILE)
        assert result.returncode == 0, result.stderr
        data = json.loads(result.stdout)
        assert len(data) > 0

    def test_sort_by_quality(self):
        result = run_cli("score", SAMPLE_RULES, "--sort-by", "quality")
        assert result.returncode == 0
        data = json.loads(result.stdout)
    ...
    def test_sort_by_false_positive(self):
        result = run_cli("score", SAMPLE_RULES, "--sort-by", "false_positive")
        assert result.returncode == 0
        data = json.loads(result.stdout)
    ...
    def test_min_quality_filter(self):
        result = run_cli("score", SAMPLE_RULES, "--min-quality", "20")
        assert result.returncode == 0
        data = json.loads(result.stdout)
        for r in data:
            assert r["quality"] >= 20
    ...
    def test_max_fp_filter(self):
        result = run_cli("score", SAMPLE_RULES, "--max-fp", "5")
        assert result.returncode == 0
        data = json.loads(result.stdout)
        for r in data:
    ...
    def test_output_to_file(self, tmp_path):
        ...
        result = run_cli("score", SAMPLE_RULES, "--output", out_file)
        assert result.returncode == 0
    ...
    def test_nonexistent_file(self):
        result = run_cli("score", "nonexistent.rules")
        assert result.returncode != 0
